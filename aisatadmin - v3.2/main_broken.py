...
class WebEngineView(QWebEngineView):
    ...
    def process_pdf_export(self, html, view):
        ...
        try:
            ...
            if match:
                ...
                filename = export_data.get('path', f'Compliance_Report_{timestamp}.pdf')
    ...
    def generate_pdf_html(self, export_data):
        """Generate HTML content for the PDF"""
        title = export_data.get('title', 'Compliance Report')
        filters = export_data.get('filters', {})
        stats = export_data.get('stats', '')
        controls = export_data.get('controls', [])
        ...
        if controls:
            for control in controls:
                status_class = control.get('status', '').lower()
                status_icon = '<div class="status-indicator status-passing"></div>' if status_class == 'passing' else '<div class="status-indicator status-failing"></div>'
                html += f"""
                    <tr class="{status_class}">
                        <td>{status_icon} {control.get('status', '')}</td>
                        <td>{control.get('id', '')}</td>
                        <td>{control.get('name', '')}</td>
                        <td>{control.get('details', '')}</td>
                    </tr>
                """
        else:
            html += """
                <tr>
                    <td colspan="4" style="text-align: center;">No controls found</td>
                </tr>
            """
        ...
        html += f"""
            ...
            <div class="footer">
                ...
                <p>This report contains {len(controls) if controls else 0} controls</p>
            </div>
        </body>
        </html>
        """
...
class AdminPanel(QMainWindow):
    ...
    def initUI(self):
        ...
        # Load the new compliance dashboard design
        sidepanel_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sidepanel.html")
        if os.path.exists(sidepanel_path):
            self.loadCompliance()
        else:
            # Fall back to original layout if sidepanel.html doesn't exist
            self.loadOriginalUI(main_layout)

    def loadCompliance(self):
        """Load the modern compliance dashboard from sidepanel.html."""
        # Get current token
        token = self.settings.value("auth_token", "")

        # Create a full-screen web view for the compliance dashboard
        ...
        # Connect custom message handlers
        self.web_view.page().javaScriptConsoleMessage = self.handleConsoleMessages

    def handleConsoleMessages(self, level, message, line, source):
        """Handle messages from the console in the web view."""
        if '{' in message and '}' in message and 'type' in message:
            try:
                ...
                if json_match:
                    ...
                    if msg_type == 'menuClick':
                        ...
                        # Here you could add logic to handle different menu items
    ...
    def loadOriginalUI(self, main_layout):
        """Load the original UI layout from previous version."""
        ...
        menu_categories = [
            {"category": "Requests", "items": [
                {"name": "Pending Request", "icon": "admin/wall-clock.png"},
                {"name": "Rejected Request", "icon": "admin/file.png"},
                {"name": "Scheduled Request", "icon": "admin/schedule.png"},
                {"name": "Schedule", "icon": "admin/booking.png"},
                {"name": "Request Ticket", "icon": "admin/ticketing.png"},
            ]},
            {"category": "Users", "items": [
                {"name": "Users", "icon": "admin/group.png"},
                {"name": "Priority Users", "icon": "admin/priority.png"},
            ]},
            {"category": "Management", "items": [
                {"name": "Transaction History", "icon": "admin/transaction-history.png"},
                {"name": "Announcement Management", "icon": "admin/megaphone.png"},
            ]},
        ...
        # Add categorized buttons for each menu item
        self.buttons = {}
        for cat in menu_categories:
            # Category header
            cat_label = QLabel(cat["category"])
            cat_label.setStyleSheet("color: #b0b8c1; font-size: 13px; font-weight: bold; margin: 18px 0 6px 8px;")
            ...
            for item in cat["items"]:
                ...
                """)
                if item["icon"]:
                    icon_path = os.path.join("img", item["icon"])
                    if os.path.exists(icon_path):
